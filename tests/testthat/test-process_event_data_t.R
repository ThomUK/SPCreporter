test_that("process_event_data_t: happy path", {
...
test_that("process_event_data_t: single event produces one row (time from event to cutoff)", {
  e_data <- tibble::tibble(
    ref = "1",
    measure_name = "Name",
...
test_that("process_event_data_t: events with identical datetimes produce a zero time-between row", {
  e_data <- tibble::tibble(
    ref = "1",
    measure_name = "Name",
...
test_that("process_event_data_t: passing in an empty event list returns NULL", {
