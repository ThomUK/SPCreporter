...
```{r map-over-data}
expand_template <- \(...) knitr::knit_expand("details_template.Rmd", ...)
knit_out_list <- data_bundle_full |>
  pmap(expand_template, .progress = "HTML charts")

knit_out <- knit_child(text = knit_out_list, quiet = TRUE)
```
