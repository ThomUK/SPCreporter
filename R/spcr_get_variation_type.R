...
spcr_get_variation_type <- function(spc, improvement_direction) {
  ...
  if (variation == "common_cause") {
    return("CC")
  }

  if (variation == "special_cause_improvement" && tolower(improvement_direction) == "increase") {
    return("SC_HI_IMP")
  }
  if (variation == "special_cause_improvement" && tolower(improvement_direction) == "decrease") {
    return("SC_LO_IMP")
  }

  if (variation == "special_cause_concern" && tolower(improvement_direction) == "increase") {
    return("SC_LO_CON")
  }
  if (variation == "special_cause_concern" && tolower(improvement_direction) == "decrease") {
    return("SC_HI_CON")
  }

  if (variation == "special_cause_neutral" && relative_to_mean == -1) {
    return("SC_LO_NEUTRAL")
  }
  if (variation == "special_cause_neutral" && (relative_to_mean == 1 | relative_to_mean == 0)) {
